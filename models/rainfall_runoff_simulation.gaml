...
global {
	// The model is simulation ready. However, to check the calibration, set the following calibration_state <- true and 
	//	uncomment data plot(original water level) under the display "Measured Water Level" in experiment block at the bottom
	...
	file common_river_net_shapefile <- file("../includes/river_network/common/common_net_buff.shp"); //bagmati river
	...
	//Diffusion rates based on slope
	float low_slope_diffusion_rate <- 0.3;
	float medium_slope_diffusion_rate <- 0.6;
	float high_slope_diffusion_rate <- 0.9;
	
	// water input from starting point of river
	float constant_river_water_input <- 2.25;
	
	//water_height is reduced from the cell using infiltration_coeff
	float infiltration_coeff <- 0.005;
	
	// water_height is exagerrated with the water_adjustment_factor
	float water_adjustment_factor <- 2.0;
	//
	int hour_steps <- 60;
	...
	//initialize rainfall and water_level data
	matrix rainfall_data;
	matrix water_level_data;
	
	//initialize water input point of rivers
	cell dhap_dam_cell;
	cell bagdwar_cell;
	...
	//measured water level value in model
	...
	init {
		...
		//dhap dam water flow start point: actual elevation=2098.741245738319776, 5 is added to makee the point visible in model
		create dhap_dam_point number: 1 {
			location <- point(to_GAMA_CRS({939112.16664863564074, 3083560.629052739124745, 2098 + 5}, "EPSG:32644"));
		}

		//Bagdwar water flow start point actual height: 2514.674001746269823, 5 is added to makee the point visible in model
	...
	//action to assign altitude based on dem pixel value and find neighbors cell
	action init_cells {
		ask cell {
			altitude <- grid_value;
	...
	action init_river_water {
		...
		ask cell overlapping common_river_geom {
			water_height <- constant_river_water_input * (water_adjustment_factor); 
		}

		ask cell overlapping bhagdwar_river_geom {
			water_height <- (constant_river_water_input * 0.67) * (water_adjustment_factor);
		}

		ask cell overlapping dhap_dam_river_geom {
			water_height <- (constant_river_water_input * 0.33) * (water_adjustment_factor);
		}
	...
	// Water is added to river branches based on their percentage contribution of water to form the combined river
	reflex add_water_in_start_point {
		ask dhap_dam_cell {
			water_height <- (constant_river_water_input * 0.33) * (water_adjustment_factor);
		}
	...
	reflex adding_input_water {
		...
		if (hour_steps_new = steps_count) {
			hourly_water_input <- (float(rainfall_data[2, hour_count]) / 1000) * (water_adjustment_factor);
			...
			hourly_water_input <- (float(rainfall_data[2, 0]) / 1000) * (water_adjustment_factor);
			water_input <- hourly_water_input / hour_steps;
		}

		//To plot value in chart
...
grid cell file: dem_file neighbors: 8 frequency: 0 use_regular_agents: false use_individual_shapes: false use_neighbors_cache: false schedules: [] {
	...
	action flow {
		...
		if (water_height > 0) {
			...
			//	get all the cells already done
			...
			//	compute the height of the neighbours cells according to their altitude and water_height
				...
				// sorting ensures to prioritize the water flow starting from neighbor with the lowest height 
...
experiment Run type: gui {
	...
	parameter "Input file" var: rainfall_file category: "Rainfall Data (.csv)";
